...
Each visual acceptance criteria was verified in the rebuilt `Prototype01.unity` scene:

- **V01 (Combat Zone Layout):** PASS. Viewport occupies center screen (`Y: 700 to 1540`); orthographic camera frames Hero and Monster with cinematic spacing.
- **V02 (Hero HP / Shield / Rage):** PASS. Jade health bar, blue Qi shield overlay, and amber rage bar render accurately with real-time numeric readouts.
- **V03 (Monster/Boss HP):** PASS. Crimson boss health bar renders at top of viewport with gold border, skull icon, and percentage text.
- **V04 (Status Icons):** PASS. Stun, Root, Freeze, and Anti-CC badges appear dynamically with radial cooldown sweeps.
- **V05 (Cast Bar):** PASS. Cyan progress bar appears under Hero during skill cast time, displaying skill name and countdown.
- **V06 (Channel Bar):** PASS. Purple focus bar appears during channeled skills with tick pulses; displays red banner if interrupted.
- **V07 (Skill Action Bar):** PASS. 5 slots rendered with mobile portrait spacing (Basic, 3 Skills, Large Ultimate).
- **V08 (Cooldown Presentation):** PASS. Cooldown sweep overlay dims slot with clean countdown text (`{sec:F1}s`).
- **V09 (RageCost & Ultimate Pulse):** PASS. RageCost badges displayed on skill slots; golden breathing excitation pulse triggers on Ultimate at `>= 100` Rage.
- **V10 (Auto Control):** PASS. Button reflects active state (`#F1C40F`) and dispatches `BattleManager.Instance.SetAutoBattle`.
- **V11 (Speed Control):** PASS. Toggles `1X`/`2X` display state without altering `Time.timeScale`.
- **V12 (Companion HUD):** PASS. Gracefully hides when companion is absent; displays `TRỌNG THƯƠNG` on death without fake timers.
- **V13 (Damage Popup):** PASS. Floating typography scales and colors distinctively for Normal, Skill, Crit, Dodge, and Shield Absorb.
- **V14 (Debug UI Separation):** PASS. Developer debug controls cleanly collapsed into bottom slide-up drawer.
- **V15 (Safe Area & Navigation):** PASS. UI elements bounded within device safe area; bottom navigation tabs properly aligned.
